const xx_danwei = {
        template: `
                <scroll :data="listData" :pullup="pullup" @scrollToEnd="loadMore" class="xx_danwei overhid" >
                    <ul>
    ...
    `,
                            ...
                            .then(res => {
                                ...
                                if (results.length != 0) {
                                    results.forEach((value, index) => { //处理三级数据
                                        ...
                                        that.listData.push({
                                            address: '/index/xxjd/xx_list/' + value.id,
                                            time: value.name,
                                            query: '单位轴',
                                        })
        ...
        mounted() {
            this.$store.commit('commonModules/getHeight', '.xx_danwei');
            this.loadMore();
        }
    }
    //