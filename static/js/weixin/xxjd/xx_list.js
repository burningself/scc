const xx_list = {
    template: `
            <scroll :data="listData" :pullup="pullup" @scrollToEnd="loadMore" class="xx_list overhid" >
                <ul>
                            ...
                            if (results.length != 0) {
                                results.forEach((value, index) => { //处理三级数据
                                    ...
                                    that.listData.push({
                                        ...
                                        address: '/index/xxjd/xx_list_gc/' + this.$route.params.num + '_' + value.id,
                                        time: value.PrjName,
                                        query: '单位轴',
                                    })
                                })
    ...
    mounted() {
        this.$store.commit('commonModules/getHeight', '.xx_list');
        this.loadMore();
    }
}