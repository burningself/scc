const gx_list_gc_gxList = {
    template: `
        <scroll class="gx_list_gc_gxList overhid"  :data="listData">
    ...
    mounted() {
        ...
        this.$store.commit('commonModules/getHeight', '.gx_list_gc_gxList');
        const par = this.$route.params,
            that = this,
            pId = par.num;
        // this.$store.commit('commonModules/changeTitle', '工序轴');
                ...
                if (results.length != 0) {
                    results.forEach(v => {
                        ...
                        this.listData.push({
                            ...
                            query: v.id + '_indexUnit', //地址参数, 什么轴+工序id+项目id
                            id: v.id
                        })
